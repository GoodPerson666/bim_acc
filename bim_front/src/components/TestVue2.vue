<template>
  ...
  <div class="top-bar">
    ...
    <el-button-group>
      ...
      <el-button @click="getLocalId">获取 Local ID</el-button>
    </el-button-group>
  </div>
  ...
  <el-dialog
      ...
      align-center
  >
    <p><b>构建名称：</b>{{ ifcName }}</p>
    <p><b>规范条文：</b>{{ ifcRule }}</p>
    <template #footer>
...
const showPanel = ref(false) // ✅ 控制移动端面板显隐
...
function setupRaycasting() {
  ...
  containerRef.value.addEventListener('click', async (event) => {
    ...
    if (result) {
      if (localId.value !== null && localId.value !== LOCKED_ID) {
...
async function getLocalId() {
  const targetGuids = [
    '1wXU_jTED61P7ZYfArwKmh',
    '1wXU_jTED61P7ZYfArwKmy',
  ]
  const localIds = await model.getLocalIdsByGuids(targetGuids)
  const result = {}
  targetGuids.forEach((guid, index) => {
    result[guid] = localIds[index]
  })
  console.log('GUID → localId 映射结果：', result)
  return result
}
